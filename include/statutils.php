...
function getDailyStats( $year, $month )
{
  ...
  while (list($year,$month,$date,$hits) = $xoopsDB->fetchRow($result))
  {
    ...
    $dailylist[$i]['graph'] = "<img src=\"images/leftbar.gif\" height=\"$l_size[1]\" width=\"$l_size[0]\" Alt=\"$date - $hits\"><img src=\"images/mainbar.gif\" Alt=\"$date - $hits\" height=\"$m_size[1]\" width=\"$midbarsize\"><img src=\"images/rightbar.gif\" height=\"$r_size[1]\" width=\"$r_size[0]\" Alt=\"$date - $hits\">";
    $i++;
  }
  $xoopsDB->freeRecordSet($result);
...
function getHourlyStats( $year, $month, $date )
{
  ...
  for ($k = 0;$k<=23;$k++)
  {
    ...
    $midbarsize = (($hits == 0) ? 0 : substr(100 * $hits / $totalhitshour, 0, 5) );
    $hourlist[$k]['hits'] = $hits;
    $hourlist[$k]['percent'] = $midbarsize."%";
    $hourlist[$k]['graph'] = "<img src=\"images/leftbar.gif\" height=\"$l_size[1]\" width=\"$l_size[0]\" Alt=\"$hour - $hits\"><img src=\"images/mainbar.gif\" Alt=\"$hour - $hits\" height=\"$m_size[1]\" width=\"$midbarsize\"><img src=\"images/rightbar.gif\" height=\"$r_size[1]\" width=\"$r_size[0]\" Alt=\"$hour - $hits\">";
  }
