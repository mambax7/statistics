...
include_once "../class/clsWhois.php";
...
function remoteAddr()
{
  ...
  echo "</td></tr></table>\n";
  echo "<table>\n";
  echo "<tr><th>".STATS_REMOTE_IP."</th><th>"
      .STATS_REMOTE_DATE."</th><th>".STATS_REMOTE_HOUR."</th><th>"
      .STATS_REMOTE_HITS."</th></tr>\n";
  foreach( $iplist as $item )
  {
    echo "<tr><td><a href=\"index.php?op=reverseip&amp;iplookup=".$item['ip']."\">".$item['ip']."</a></td>"
	    ."<td>".$item['ipmonth']."-".$item['ipday']."-".$item['ipyear']."</td><td>".$item['iphour']."</td><td>".$item['hits']."</td></tr>\n";
  }
...
function referDB( $orderby )
{
  ...
  $referimg = $orderby == "refer" ? "arrowup.gif" : "arrowdn.gif";
  $hitsimg = $orderby == "hits" ? "arrowup.gif" : "arrowdn.gif";
  $dateimg = $orderby == "date" ? "arrowup.gif" : "arrowdn.gif";

  echo "<div style=\"font-size: xx-small;\"><table>\n";
  echo "<tr><th>".STATS_REMOTE_IP."</th><th>".STATS_REFER.": <A href=\"index.php?op=refer&amp;orderby=refer\"><img src=\"../images/".$referimg."\"></a></th>"
      ."<th>".STATS_XWHOIS."</th><th>".STATS_REFER_PATH."</th><th>\n"
  ...
  foreach( $referlist as $item )
  {
    ...
    echo "<tr><td align='left'><a href=\"index.php?op=reverseip&amp;iplookup=".$item['ip']."\">".$item['ip']."</a></td>"
	    ."<td align='right'><a href='http://".$item['refer']."' target='_new'>".$item['refer']."</a></td>\n"
...
switch( $op )
{
  ...
  case 'purge_ips':
    if(isset($_POST['confirm'])  && $_POST['confirm'] == "purge_ips" ) 
    {
      purgeRemoteAddr();
    }
    else
    {
      $hidden = array( confirm => 'purge_ips',
                       op => 'purge_ips' );
      xoops_confirm( $hidden, "index.php", STATS_REMOTEADDR_PURGESURE, STATS_IPPURGE );
    }
    echo "<hr><a href=\"index.php\">".STATS_ADMINHEAD."</a>\n";
    break;
  ...
  case 'purge_refer':
    if(isset($_POST['confirm'])  && $_POST['confirm'] == "purge_refer" ) 
    {
      purgeReferDB();
    }
    else
    {
      $hidden = array( confirm => 'purge_refer',
                       op => 'purge_refer' );
      xoops_confirm( $hidden, "index.php", STATS_REFER_PURGESURE, STATS_REFERPURGE );
    }
    echo "<hr><a href=\"index.php\">".STATS_ADMINHEAD."</a>\n";
    break;
  ...
  case 'blacklist_refer':
    ...
    {
      $hidden = array( confirm => 'blacklist_refer',
                       op => 'blacklist_refer',
                       blr => $_POST['bad_refer'] );
      xoops_confirm( $hidden, "index.php", STATS_REFER_BLSURE, STATS_REFERBLACKLIST );
    }
    elseif ( isset($_POST['confirm'])  && $_POST['confirm'] == "blacklist_refer") 
    {
  ...
  case 'purge_blacklist':
    if(isset($_POST['confirm'])  && $_POST['confirm'] == "purge_blacklist" ) 
    {
      purgeBlacklist();
    }
    else
    {
      $hidden = array( confirm => 'purge_blacklist',
                       op => 'purge_blacklist' );
      xoops_confirm( $hidden, "index.php", STATS_REFER_PURGEBL, STATS_PURGEBL );
    }
    echo "<hr><a href=\"index.php\">".STATS_ADMINHEAD."</a>\n";
    break;
  ...
  case 'purge_userscreen':
    if(isset($_POST['confirm'])  && $_POST['confirm'] == "purge_userscreen" ) 
    {
      purgeUserScreen();
    }
    else
    {
      $hidden = array( confirm => 'purge_userscreen',
                       op => 'purge_userscreen' );
      xoops_confirm( $hidden, "index.php", STATS_REFER_PURGEUS, STATS_SCREEN_PURGE );
    }
    echo "<hr><a href=\"index.php\">".STATS_ADMINHEAD."</a>\n";
    break;
